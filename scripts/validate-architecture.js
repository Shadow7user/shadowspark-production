...
const RULES = [
  ...
  },
  {
    ...
    description:
      "Avoid direct fetch calls to external URLs in client components",
    check: (content, filePath) => {
      if (!content.includes("use client")) return false;
    ...
    suggestion:
      "Use tRPC, internal /api routes, or server actions instead of external fetch",
  },
];
...
export { RULES, scanDirectory, validateFile };
